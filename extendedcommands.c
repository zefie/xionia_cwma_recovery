...
char* choose_file_menu(const char* directory, const char* directory2, const char* fileExtensionOrDirectory, const char* headers[])
{
    char path[PATH_MAX] = "";
    DIR *dir;
    struct dirent *de;
    ...
    if (directory2 != NULL) {
	dir_len2 = strlen(directory2);
	files2 = gather_files(directory2, fileExtensionOrDirectory, &numFiles2);
    	if (fileExtensionOrDirectory != NULL) {
        	dirs2 = gather_files(directory2, NULL, &numDirs2);
	}
	total2 = (numDirs2 + numFiles2);
    }
    ...
    {
        char** list = (char**) malloc((total3 + 1) * sizeof(char*));
        list[total3] = NULL;
	...
	int j = 0;
        ...
        for (i = 0 ; i < numDirs; i++)
        {
            list[j] = strdup(dirs[i] + dir_len);
	    j++;
        }

        for (i = 0 ; i < numDirs2; i++)
        {
            list[j] = strdup(dirs2[i] + dir_len2);
	    j++;
        }

        for (i = 0 ; i < numFiles; i++)
        {
            list[j] = strdup(files[i] + dir_len);
	    j++;
        }

        for (i = 0 ; i < numFiles2; i++)
        {
            list[j] = strdup(files2[i] + dir_len2);
	    j++;
        }
	    ...
	    char* subret = NULL;
            if (chosen_item < (numDirs + numDirs2))
            {
		if (chosen_item >= numDirs) {
	                subret = choose_file_menu(dirs2[(chosen_item-numDirs)], NULL, fileExtensionOrDirectory, headers);
		} else {
	                subret = choose_file_menu(dirs[chosen_item], NULL, fileExtensionOrDirectory, headers);
		}
                if (subret != NULL)
                {
                    strcpy(ret, subret);
	    ...
	    if ((chosen_item-(numDirs + numDirs2)) >= numFiles) {
	            strcpy(ret, files2[((chosen_item-(numDirs + numDirs2))-numFiles)]);
	    } else {
	            strcpy(ret, files[(chosen_item-(numDirs + numDirs2))]);
	    }
            return_value = ret;
            break;
        }
    ...
    free_string_array(files2);
    free_string_array(dirs);
    free_string_array(dirs2);
    return return_value;
}
...
void show_choose_zip_menu()
{
    ...
    char* file = choose_file_menu("/sdcard/", NULL, ".zip", headers);
    if (file == NULL)
        return;
...
void show_nandroid_restore_menu()
{
    ...
    char* file = choose_file_menu("/sdcard/clockworkmod/backup/", "/sdcard/nandroid/mem=477M/", NULL, headers);
    if (file == NULL)
        return;
...
void show_nandroid_advanced_restore_menu()
{
    ...
    char* file = choose_file_menu("/sdcard/clockworkmod/backup/", "/sdcard/nandroid/mem=477M/", NULL, advancedheaders);
    if (file == NULL)
        return;
...
void handle_failure(int ret)
{
    ...
    ui_print("/tmp/recovery.log was copied to /sdcard/clockworkmod/recovery.log. Please attach this file to your post when reporting to zefie.\n");
}
